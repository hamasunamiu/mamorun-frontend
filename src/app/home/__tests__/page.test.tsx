...
describe("CareHomePage", () => {
  ...
  describe("ToDo関連", () => {
    ...
    test("UT-F-206: 正しいタスク名で新規追加するとtodosに追加されModalが閉じる", async () => {
      ...
      (useCareHomeData as jest.Mock).mockReturnValue(
        createMockHookReturn({ setTodos: mockSetTodos }),
      );
      ...
      // setTodosが「現在のtodosに新しいtodoを追加する関数」として呼ばれたか確認する
      expect(mockSetTodos).toHaveBeenCalledWith(expect.any(Function));

      // Modalが閉じたことを確認（タイトルが消える）
      await waitFor(() => {
    ...
    test("UT-F-210: ToDo完了切り替え：完了→未完了", async () => {
      ...
      // 完了状態なのでaria-labelは「完了を取り消す」系の文言になっているはず（未確認）
    ...
    test("UT-F-212: ToDo削除確認すると該当Todoがtodosから除外される", async () => {
      ...
      // 確認モーダル内の「削除する」的なボタンを押す（実際のラベルは未確認）
  ...
  test("UT-F-219: 予定日が本日（境界値）の場合バリデーションを通過する", async () => {
    ...
    (useCareHomeData as jest.Mock).mockReturnValue(
      createMockHookReturn({ setSchedules: mockSetSchedules }),
    );
    ...
    await user.click(screen.getByRole("button", { name: "予定を追加" }));
    await user.type(screen.getByLabelText(/タイトル/), "フィラリア薬");
    // 「今日」を固定するため、テストの実行日に依存しないよう少し先の日付ではなく
    // システム時刻をそのまま使う（2026-06-29を想定）
    await user.type(screen.getByLabelText(/予定日/), "2026-06-29");
    await user.click(screen.getByRole("button", { name: "追加する" }));

    // バリデーション通過 → setSchedulesが呼ばれてModalが閉じる
    expect(mockSetSchedules).toHaveBeenCalledWith(expect.any(Function));
  });
...
describe("条件表示", () => {
  test("UT-F-230: pet.hospital_phoneが存在する場合EmergencyCallButtonが表示される", () => {
    render(<CareHomePage />);
    expect(screen.getByRole("link", { name: "緊急発信" })).toBeInTheDocument();
  });
  ...
  test("UT-F-232: isMounted=trueの場合日付ラベルが表示される", () => {
    render(<CareHomePage />);
    expect(screen.getByText(/2026年6月29日/)).toBeInTheDocument();
  });
